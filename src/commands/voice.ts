import { Discord, Guard, GuardFunction, On, Slash, SlashOption } from "discordx";
import { CommandInteraction, GuildMember, Message, MessageEmbed, MessagePayload, TextBasedChannels, VoiceState } from "discord.js";
import { log } from "../logging";
import { AudioPlayer, AudioPlayerError, AudioPlayerStatus, AudioResource, createAudioPlayer, createAudioResource, getVoiceConnection, joinVoiceChannel, VoiceConnectionStatus } from "@discordjs/voice";
...
export abstract class voice {
    ...
    async play(@SlashOption("media", { description: "The media to play", required: true }) url: string,
        ...
        try {
                ...
                this.player.on("stateChange", async (oldState, newState) => {
                    if (oldState.status === AudioPlayerStatus.Playing && newState.status === AudioPlayerStatus.Idle) {
                        ...
                        if (this.queue.hasMedia()) {
                            ...
                            this.statusMsg = await this.lastChannel.send("Now playing " + meta.title + " [" + meta.queuedBy + "]");
                        } else {
                            if (this.statusMsg !== undefined) this.statusMsg.delete();
                            this.statusMsg = await this.lastChannel.send("Reached end of queue, stoped playing");
                        }
                    }
                });
            }
            ...
            if (youtubeId !== undefined) {
                ...
                metadata.url = url;
                metadata.queuedBy = (await interaction.guild.members.fetch(interaction.user.id)).displayName;
            ...
            if (this.player.state.status !== AudioPlayerStatus.Playing) {
                ...
                embed.description = (audioResource.metadata as IMetadata).title + " queued";
            }
    ...
    async viewQueue(interaction?: CommandInteraction): Promise<void> {
        try {
            ...
            const embed = new MessageEmbed();
            const title = this.player.state.status == AudioPlayerStatus.Playing ? "Now Playing" : "Current Queue";
            ...
            if (this.queue.hasMedia()) {
                ...
                for (let i = 0; i < queuedSongs.length; i++) {
                    ...
                    description += "\n" + (i + 1).toString() + ". " + meta.title + " [" + meta.queuedBy + "]";
                }
            } else {
    ...
    async skip(@SlashOption("index", { description: "The index of the song to skip to" }) skip: string, interaction: CommandInteraction): Promise<void> {
        try {
            await interaction.deferReply();
            var i = parseInt(skip);
            const embed = new MessageEmbed();
